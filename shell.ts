...
import { encodeToString as toHex } from "https://deno.land/std/encoding/hex.ts";


const SQLITE3_PROGRAM = 'sqlite3';
const SQLITE3_ARGS = ['-batch', '-noheader', '-json', '-cmd', '.binary on',
  '-cmd', '.print *']
...
function bindPositional (query: string, params: ParametersArray) {
  let idx = 0;
  const rv = query.replace(/['?]/g, paramReplacer(function (_) {
    const i = idx++;
...
function bindNamed (query: string, params: ParametersObject) {
  return query.replace(/('|[@:$]\w+)/g, paramReplacer(function (match) {
