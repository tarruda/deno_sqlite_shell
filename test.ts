...
import { bindParams, connect, formatParam, Shell } from "./mod.ts";
...
async function setup() {
  ...
  for (
    ...
    await shell.execute(
      `INSERT INTO dbz_chars (name, power) VALUES (?, ?)`,
      [name, power],
    );
  }
...
Deno.test("formatParam", () => {
  ...
  assertEquals(formatParam("false"), "'false'");
  assertEquals(
    formatParam("some 'string' with quotes"),
    "'some ''string'' with quotes'",
  );
  assertEquals(
    formatParam(new Uint8Array([0xde, 0xad, 0xbe, 0xef])),
    "x'deadbeef'",
  );
});
...
Deno.test("bindParams positional", () => {
  ...
  assertEquals(
    bindParams(
      "c1 = ? AND c2 = '?' AND c3 = ? AND c4 = ?",
      [4.5, 2.3, new Uint8Array([0, 1, 2, 3, 4, 5])],
    ),
...
Deno.test("bindParams positional with less values than placeholders", () => {
  assertThrows(
    ...
    Error,
    `Cannot find parameter with index "2"`,
  );
});
...
Deno.test("bindParams positional with more values than placeholders", () => {
  assertThrows(
    ...
    Error,
    `Not all parameters were used by "c1 = ? AND c2 = '?' AND c3 = ? AND c4 = ?"`,
  );
});
...
Deno.test("bindParams named", () => {
  ...
  assertEquals(
    bindParams(
      "c1 = :c1 AND c2 = ':c2' AND c3 = :c3 AND c4 = :c4",
      { c1: true, c2: "unused", c3: false, c4: "some 'quotes' in str" },
    ),
  ...
  assertEquals(
    bindParams(
      "c1 = :c1 AND c2 = ':c2' AND c3 = :c3 AND c4 = :c4",
      { c1: 4.5, c3: 2.3, c4: new Uint8Array([0, 1, 2, 3, 4, 5]) },
    ),
...
Deno.test("bindParams named missing", () => {
  assertThrows(
    ...
    Error,
    `Cannot find parameter with key "c3"`,
  );
});
...
Deno.test("query while already processing query", async () => {
  ...
  for await (const _ of shell.queryRaw("SELECT * FROM dbz_chars")) {
    await assertThrowsAsync(
      ...
      Error,
      "Already processing another query",
    );
  }
